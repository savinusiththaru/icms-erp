...
export default function ClientsPage() {
    ...
    // Filter only Clients
    const filteredContacts = contacts.filter(c => c.type === 'Client');
        ...
        <div style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    ...
                    <p style={{ color: 'var(--text-muted)' }}>Manage your client relationships</p>
                </div>
                    ...
                    filteredContacts.map(contact => (
                        <div key={contact.id} className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px', position: 'relative' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <div style={{
                                    width: '48px', height: '48px', borderRadius: '50%', background: '#dbeafe', // Blue for clients
                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                    fontSize: '1.2rem', fontWeight: '600', color: '#1d4ed8'
                                }}>
                                    {contact.name[0]}
                                </div>
                                <div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Client">
                <form onSubmit={handleCreate} className={formStyles.form}>
                    <div className={formStyles.formGroup}>
                        ...
                        <input className={formStyles.input} required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="John Doe" />
                    </div>
                    <div className={formStyles.formGroup}>
                        <label className={formStyles.label}>Company</label>
                        <input className={formStyles.input} required value={formData.company} onChange={e => setFormData({ ...formData, company: e.target.value })} placeholder="Acme Inc." />
                    </div>

                    <div className={formStyles.row}>
                        <div className={formStyles.formGroup}>
                            <label className={formStyles.label}>Email</label>
                            <input className={formStyles.input} type="email" required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} placeholder="email@example.com" />
                        </div>
                        <div className={formStyles.formGroup}>
                    ...
                    <div className={formStyles.formGroup}>
                        ...
                        <input className={formStyles.input} value={formData.address} onChange={e => setFormData({ ...formData, address: e.target.value })} placeholder="123 Main St, City" />
                    </div>

                    {/* Hidden Type Field - Always Client */}
                    ...
                    <div className={formStyles.actions}>
                        ...
                        <button type="submit" className={formStyles.btnSubmit}>Add Client</button>
                    </div>
                </form>
            </Modal>
        </div>
